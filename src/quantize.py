...
BASE_SAVE_PATH = f"{RUNS_VOL_PATH}/{BASE_HF_MODEL.split('/')[-1]}-AWQ"
...
def helper(model_path, quant_config, save_path, save_hub):
    ...
    text = processor.apply_chat_template(
        cal_ds, tokenize=False, add_generation_prompt=True
    )
...
def main(base: bool, sft: bool, dpo: bool):
...
def run(base: bool, sft: bool, dpo: bool):
    main(base, sft, dpo)


@app.local_entrypoint()
def local(base: bool = False, sft: bool = False, dpo: bool = False):
    run.remote(base, sft, dpo)
...
if __name__ == "__main__":
    ...
    main(args.base, args.sft, args.dpo)
